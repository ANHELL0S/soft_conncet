...
import { RatingMetrics } from '../../components/rating/RatingMetrics.jsx'
import { StarRatingDisplay } from '../../components/rating/StarRatingDisplay.jsx'
import {
	ArrowLeft,
	MessageSquare,
	CheckCircle,
...
const PostHeader = ({ post, ratingData }) => {
	return (
		<div className='flex items-start gap-3'>
			<div className='flex-1 min-w-0 space-y-4'>
				...
				{/* Rating resumido en el header */}
				{ratingData?.average > 0 && (
					<div className='flex items-center gap-2'>
						<StarRatingDisplay rating={ratingData.average} total={ratingData.total} size='sm' />
					</div>
				)}
...
const PostDetail = () => {
	...
	// Efecto para cargar rating data
	useEffect(() => {
		if (id) {
			const data = getFullRatingData(id)
			setRatingData(data)
		}
	}, [id, getFullRatingData])

	// Efecto para escuchar cambios en el rating
	useEffect(() => {
	...
	// Efecto para abrir el modal de respuesta automáticamente si viene el parámetro
	useEffect(() => {
		const shouldOpenAnswer = searchParams.get('openAnswer') === 'true'
		if (shouldOpenAnswer && isAuthenticated) {
			setShowAnswerModal(true)
			setSearchParams({}, { replace: true })
		} else if (shouldOpenAnswer && !isAuthenticated) {
			navigate('/auth/signin', { state: { from: `/post/${id}?openAnswer=true` } })
		}
	}, [searchParams, isAuthenticated, id, navigate, setSearchParams])
						...
						<div className='lg:col-span-2'>
							<article className='space-y-6'>
								<div className='bg-gray-50 p-4 rounded-2xl text-gray-600'>
									<PostHeader post={post} ratingData={ratingData} />
									<PostActions
										hasLiked={hasLiked}
										...
										{answers.map(answer => (
											<AnswerCard
												...
												onAccept={async answerId => {
													console.log('Aceptar respuesta:', answerId)
													refetch()
													refetchAnswers()
												}}
											/>
